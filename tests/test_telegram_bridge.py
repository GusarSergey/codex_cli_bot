...
def test_load_ask_question_api_falls_back_without_claude(monkeypatch) -> None:
    import builtins

    original_import = builtins.__import__

    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "runners.claude" and level == 2:
    ...
    ) = telegram_loop._load_ask_question_api()

    assert anyio.run(answer_ask_question, "req", "hello") is False
    assert anyio.run(answer_ask_question_with_options, "req") is False
    assert format_question_message(object()) == ""
    assert get_ask_question_flow(channel_id=123) is None
    assert get_pending_ask_request(channel_id=123) is None
    assert get_question_option_buttons(object()) == []
...
@pytest.mark.anyio
async def test_run_main_loop_handles_text_when_ask_question_api_unavailable(
    monkeypatch,
) -> None:
    runner = ScriptRunner([Return(answer="ok")], engine=CODEX_ENGINE)
    cfg = replace(make_cfg(FakeTransport(), runner), allowed_user_ids=(123,))

    async def answer_ask_question(_request_id: str, _text: str) -> bool:
        return False

    async def answer_ask_question_with_options(_request_id: str) -> bool:
        return False

    monkeypatch.setattr(
        telegram_loop,
        "_load_ask_question_api",
        lambda: (
            answer_ask_question,
            answer_ask_question_with_options,
            lambda _flow: "",
