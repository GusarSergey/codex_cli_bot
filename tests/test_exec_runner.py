...
def test_pid_appears_alive_treats_windows_invalid_handle_as_dead(
    monkeypatch,
) -> None:
    import os
    ...
    def raise_invalid_handle(_pid: int, _sig: int) -> None:
        raise OSError(9, "The handle is invalid", None, 6, None)

    monkeypatch.setattr(os, "kill", raise_invalid_handle)
    monkeypatch.setattr(os, "name", "nt", raising=False)

    assert runner_module._pid_appears_alive(12345) is False


@pytest.mark.anyio
async def test_watchdog_ignores_windows_invalid_handle(monkeypatch) -> None:
    import os

    import untether.runner as runner_module

    runner = CodexRunner(codex_cmd="codex.cmd", extra_args=[])
    runner._WATCHDOG_GRACE_SECONDS = 0.01

    def raise_invalid_handle(_pid: int, _sig: int) -> None:
        raise OSError(9, "The handle is invalid", None, 6, None)

    def fail_killpg(_pid: int, _sig: int) -> None:
        raise AssertionError("killpg should not be reached for invalid handle")

    monkeypatch.setattr(os, "kill", raise_invalid_handle)
    monkeypatch.setattr(os, "killpg", fail_killpg, raising=False)
    monkeypatch.setattr(os, "name", "nt", raising=False)
    reader_done = anyio.Event()
    stream = runner_module.JsonlStreamState(expected_session=None, did_emit_completed=True)

    await runner._subprocess_watchdog(
        proc=object(),
        stream=stream,
