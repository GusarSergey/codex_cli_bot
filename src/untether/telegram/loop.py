...
def _load_ask_question_api() -> tuple[Any, ...]:
    """Best-effort loader for Claude ask-question helpers.

    On Windows, importing the Claude runner can fail because its PTY stack
    depends on Unix-only modules like ``termios``. Telegram message routing
    should continue to work even when AskUserQuestion support is unavailable.
    """
    try:
        ...
        async def _answer_ask_question(_request_id: str, _text: str) -> bool:
            return False

        async def _answer_ask_question_with_options(_request_id: str) -> bool:
            return False

        def _format_question_message(_flow: Any) -> str:
            return ""

        def _get_ask_question_flow(*, channel_id: int) -> None:
            _ = channel_id
            return None

        def _get_pending_ask_request(*, channel_id: int) -> None:
            _ = channel_id
            return None

        def _get_question_option_buttons(_flow: Any) -> list[list[dict[str, str]]]:
            return []

        return (
            _answer_ask_question,
            _answer_ask_question_with_options,
            _format_question_message,
            _get_ask_question_flow,
            _get_pending_ask_request,
            _get_question_option_buttons,
        )
    return (
        answer_ask_question,
        answer_ask_question_with_options,
        ...
        async with anyio.create_task_group() as tg:
            ...
            async def route_message(msg: TelegramIncomingMessage) -> None:
                ...
                if text and not is_voice_transcribed:
                    ask_question_api = _load_ask_question_api()
                    if ask_question_api is None:
                        pass
                    else:
                        (
                            answer_ask_question,
