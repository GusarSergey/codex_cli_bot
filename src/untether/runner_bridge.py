...
_DEFAULT_PREAMBLE = (
    ...
    "- Git commit/push are allowed when the repo and task require them. Do not claim "
    "git is blocked unless you have actually run the relevant git command.\n"
    "- If you suspect a `.git/index.lock` problem, first verify that the file exists. "
    "Only report an index lock blocker if it is really present or the git command "
