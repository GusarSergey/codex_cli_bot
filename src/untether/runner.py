...
def _pid_appears_alive(pid: int) -> bool:
    """Best-effort liveness probe that tolerates Windows invalid-handle races.

    On Windows, ``os.kill(pid, 0)`` can raise ``OSError(winerror=6)`` after the
    child has already exited and the underlying process handle is gone. Treat
    that the same as "not running" so watchdog polling does not crash active
    Telegram runs.
    """
    import os as _os
    ...
    try:
        ...
        if _os.name == "nt" and getattr(exc, "winerror", None) == 6:
            return False
        raise
    return True
...
class JsonlSubprocessRunner(BaseRunner):
    ...
    def _should_use_blocking_subprocess_on_windows(self) -> bool:
        return os.name == "nt" and self._prefer_blocking_subprocess_on_windows
    ...
    def _run_blocking_subprocess_capture(
        self,
        *,
        cmd: list[str],
        payload: bytes | None,
    ...
    async def _run_impl_blocking_windows(
        self,
        *,
        state: Any,
        resume: ResumeToken | None,
        ...
        pid, rc, stdout_data, stderr_data = await anyio.to_thread.run_sync(
            partial(
                self._run_blocking_subprocess_capture,
                cmd=cmd,
                payload=payload,
                env=env,
                cwd=cwd,
        ...
        if stderr_data:
            stream.stderr_capture.extend(
                stderr_data.decode("utf-8", errors="replace").splitlines()
            )

        stream.proc_returncode = rc
        logger.info("subprocess.exit", pid=pid, rc=rc)
        if stream.did_emit_completed:
            return
    ...
    async def run_impl(
        ...
        if self._should_use_blocking_subprocess_on_windows():
            async for evt in self._run_impl_blocking_windows(
                state=state,
                resume=resume,
                logger=logger,
                cmd=cmd,
